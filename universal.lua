...
run(function()
        ...
        Function = function(callback)
            if callback then
                connection = RunService.Heartbeat:Connect(function()
                    ...
                    if YLevelJitter.Enabled then
                        local humanoid = character:FindFirstChildOfClass("Humanoid")
                        if humanoid then
                            local moveDirection = humanoid.MoveDirection
                            local shouldJitter = not YLevelJitterIdleOnly.Enabled or moveDirection.Magnitude == 0
                            
                            if shouldJitter then
                                local amount = YLevelJitterAmount.Value
                                local yLevel = YLevelJitterYLevel.Value
                                local yOffset = math.min(math.random(1, amount) ^ 2, yLevel)
                                hrp.CFrame = hrp.CFrame + Vector3.new(0, yOffset, 0)
                            end
                        end
                    end
                    
                    if PosJitter.Enabled then
                        local amount = PosJitterAmount.Value
                        local yLevel = PosJitterYLevel.Value
                        if random1 == 1 then
                            hrp.CFrame = hrp.CFrame + hrp.CFrame.RightVector * math.random(1, amount)
                        elseif random1 == 2 then
                            hrp.CFrame = hrp.CFrame - hrp.CFrame.RightVector * math.random(1, amount)
                        else
                            local yOffset = math.min(math.random(2, amount) ^ 2, yLevel)
                            hrp.CFrame = hrp.CFrame + Vector3.new(0, yOffset, 0)
                        end
                    end
                    ...
                    if VoidSpam.Enabled then
                        if random2 > 3 then
                            hrp.CFrame = CFrame.new(math.random(50, 100) ^ 2, -99999999, math.random(50, 100) ^ 4)
                        end
                    end
                    
                    cameraPart.CFrame = CFrame.new(oldCFrame.Position + Vector3.new(0, 1.5, 0))
                    Workspace.CurrentCamera.CameraSubject = cameraPart
                    
                    RunService.RenderStepped:Wait()
                    
                    if YLevelJitter.Enabled or PosJitter.Enabled or VoidSpam.Enabled then
    ...
    YLevelJitter = Desync:CreateToggle({
        ...
        Default = false,
        Function = function(callback)
            YLevelJitterIdleOnly.Object.Visible = callback
            YLevelJitterAmount.Object.Visible = callback
            YLevelJitterYLevel.Object.Visible = callback
        end,
        Tooltip = 'Jitters your Y position vertically'
    })
    
    YLevelJitterIdleOnly = Desync:CreateToggle({
        Name = 'Idle Only',
        Default = true,
        Function = function(callback) end,
        Darker = true,
        Visible = false,
        Tooltip = 'Only jitter when standing still'
    })
    
    YLevelJitterAmount = Desync:CreateSlider({
        Name = 'Jitter Amount',
        Min = 1,
        Max = 10,
        Default = 5,
        ...
        Darker = true,
        Visible = false,
        Tooltip = 'How much to jitter (higher = more jitter)'
    })
    
    YLevelJitterYLevel = Desync:CreateSlider({
        Name = 'Y Max',
        Min = 0,
        Max = 300,
        Default = 100,
        ...
        Darker = true,
        Visible = false,
        Tooltip = 'Maximum Y level for jitter (0-300 studs)'
    })
    
    PosJitter = Desync:CreateToggle({
        Name = 'Position Jitter',
        Default = false,
        Function = function(callback)
            PosJitterAmount.Object.Visible = callback
            PosJitterYLevel.Object.Visible = callback
        end,
        Tooltip = 'Randomly jitters your position'
    })
    
    PosJitterAmount = Desync:CreateSlider({
        Name = 'Position Jitter Amount',
        Min = 1,
        Max = 15,
        Default = 6,
        Function = function(val) end,
        Darker = true,
        Visible = false,
        Tooltip = 'How much to jitter position (higher = more jitter)'
    })
    
    PosJitterYLevel = Desync:CreateSlider({
        Name = 'Position Jitter Y Max',
        Min = 0,
        Max = 300,
        Default = 100,
        Function = function(val) end,
        Darker = true,
        Visible = false,
        Tooltip = 'Maximum Y level for position jitter (0-300 studs)'
    })
    
    VoidSpam = Desync:CreateToggle({
        Name = 'Void Spam',
        Default = false,
        Function = function(callback) end,
...
run(function()
    ...
    Rollbot = vape.Categories.Blatant:CreateModule({
        Name = 'Rollbot',
        Function = function(callback)
            if callback then
                connection = RunService.Heartbeat:Connect(function()
                    ...
                    if not hrp then return end
                    oldCFrame = hrp.CFrame
                    rollAngle = rollAngle + (RollSpeed.Value / 100)
                    hrp.CFrame = hrp.CFrame * CFrame.Angles(rollAngle, 0, 0)
                    cameraPart.CFrame = CFrame.new(oldCFrame.Position + Vector3.new(0, 1.5, 0))
                    Workspace.CurrentCamera.CameraSubject = cameraPart
                    RunService.RenderStepped:Wait()
                    hrp.CFrame = oldCFrame
        ...
        Tooltip = 'Continuously rolls your character'
    })
    
    RollSpeed = Rollbot:CreateSlider({
        Name = 'Roll Speed',
        Min = 1,
        Max = 50,
        Default = 10,
        Function = function(val) end,
        Tooltip = 'How fast your character rolls (higher = faster)'
    })
    
    LocalPlayer.CharacterAdded:Connect(function()
        if Rollbot.Enabled then
            local character = LocalPlayer.Character
            local humanoid = character:WaitForChild("Humanoid")
            Workspace.CurrentCamera.CameraSubject = cameraPart
        end
...
run(function()
    ...
    local VelocityDesync
    local connection
    
    VelocityDesync = vape.Categories.Blatant:CreateModule({
        Name = 'VelocityDesync',
        Function = function(callback)
            if callback then
                connection = RunService.Heartbeat:Connect(function()
                    ...
                    if math.random(1, 2) == 1 then
                        hrp.Velocity = hrp.CFrame.RightVector * 50 + Vector3.new(0, 99999, 0)
                    else
                        hrp.Velocity = hrp.CFrame.RightVector * -(100 ^ math.random(3, 6)) + Vector3.new(0, 99999, 0)
                    end
        ...
        Tooltip = 'Randomizes your velocity to confuse opponents'
    })
end)
